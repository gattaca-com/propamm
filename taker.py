...
async def trade_once(
    ...
    weth_units = usd_to_units(args.notional_usd, 18) // args.eth_price_usd
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--slippage-bps", type=int, default=30,
        help="Slippage tolerance applied to amountCheck / minOut.",
    )
    p.add_argument(
        "--eth-price-usd", type=int, default=2290,
        help="Reference ETH price (USD), used to size the WETH leg.",
    )
    p.add_argument(
...
async def amain() -> None:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as http:
        ...
        while True:
            try:
                await trade_once(w3, http, account, args, chain_id, iter_, state)
            except Exception as e:
